...
class DBHelper private constructor(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun getTask(id: Long): Task {
        ...
        if (cursor.moveToFirst()) {
            ...
            task = Task(id, title, date, position, timeStamp)
     ...
     */
    fun getTasksForSearch(selection: String, selectionArgs: Array<String>, orderBy: String): List<Task> {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                val modelTask = Task(id.toLong(), title, date, position, timeStamp)
                tasks.add(modelTask)
            } while (c.moveToNext())
        }
        c.close()
